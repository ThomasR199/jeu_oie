...
int count_attente = 0;
int points_avance_jacob_max = 0;
int mots_decouverts[MOTS] = {0};
...
void ParolesRick(){
  ...
  int parole = rand_max() % MOTS;
  mots_decouverts[parole] += 1;
...
int affichageMotsDecouverts(int tab[MOTS]){
  bool decouverteMot = false;
  for (int i = 0; i < MOTS && !decouverteMot; i += 1){
    if (tab[i] != 0) decouverteMot = true;
  }
  if (decouverteMot){
    printf("Mots découverts : | ");
    for (int i = 0; i < MOTS; i += 1){
        if (tab[i] == 1) printf("%d | ",i+1);
        else if (tab[i] >= 2) printf("%d (x%d) | ",i+1,tab[i]);
    }
    printf("\n");
  }
}
...
int main() {
  ...
  while (!victoire){
    ...
    if (de == 'e'){
      ...
      printf("|\n");
      printf("Record d'attente sur M. Jacob : %d case(s)\n",points_avance_jacob_max);
      affichageMotsDecouverts(mots_decouverts);
    }
    printf("\n");
    count_lancers += 1;
    if (dim == 1){ // Dimension clasique
      ...
      if (pas_1b == 6 && pas_2b >= 3 && chasseJacob == false){
        ...
        if (countPlayer <= countJacob){
          ...
          if (countPlayer-countJacob > points_avance_jacob_max) points_avance_jacob_max = countPlayer-countJacob;
        }
      } else {
  ...
  printf("Votre temps de jeu : %d heure(s) %d minute(s) %d seconde(s)\n",tf_heures,tf_minutes,tf_secondes);
  ...
  printf("|\n");
  printf("Record d'avance sur M. Jacob : %d case(s)\n",points_avance_jacob_max);
  affichageMotsDecouverts(mots_decouverts);
  return 0;
}